...
<table @if($collection->eloquentTableAttributes) {!! $collection->eloquentTableAttributes !!} @else class="table" @endif>
    <thead>
    <tr>
        ...
        @foreach($collection->eloquentTableColumns as $key => $name)
            ...
            <th {!! $collection->getHiddenColumnAttributes($key) !!}>
                @if(in_array($key, $collection->eloquentTableSort))
                    ...
                    {!! sortableUrlLink($model->getFieldLabel($key), array('field' => $key, 'sort'=>'asc')) !!}
                @elseif(array_key_exists($key, $collection->eloquentTableSort))
                    ...
                    {!! sortableUrlLink($model->getFieldLabel($key), array('field' => $collection->eloquentTableSort[$key], 'sort'=>'asc')) !!}
                @else
                    {{-- Looks like we don't have any header modifications, we'll just output the name --}}
                    {{ ucfirst($model->getFieldLabel($key)) }}
                @endif
            </th>
        @endforeach

        {{--  lets see if actions exists --}}
        @if ($collection->eloquentRowActions)
            <th colspan="{{ count($collection->eloquentRowActions) }}">
                {!! trans('eloquenttable::table.options') !!}
            </th>
        @endif
    ...
    @foreach($collection as $record)
        ...
        {{--  lets see if actions exists --}}
        @if ($collection->eloquentRowActions)
            @foreach($collection->eloquentRowActions as $key => $action)
                <td class="eloquenttable-action">
                    @if(array_key_exists($key, $collection->eloquentRowActionModifications))
                        {!! call_user_func_array($collection->eloquentRowActionModifications[$key], array($record, $action)) !!}
                    @else
                        {!! $collection->getActionButton($action, $record) !!}
                    @endif
                </td>
            @endforeach
        @endif
 ...
 {{--  lets see if actions exists --}}
@if ($collection->eloquentRowActions)
    @include('eloquenttable::delete-dialog')
@endif
